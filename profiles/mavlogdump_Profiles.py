...
def process(args):
    ...
    reduction_msgs = ['NKF*', 'XKF*', 'IMU*', 'AHR2', 'BAR*', 'ATT', 'BAT*', 'CTUN', 'NTUN', 'GP*', 'IMT*', 'MAG*',
                      'PL', 'POS', 'POW*', 'RATE', 'RC*', 'RFND', 'UBX*', 'VIBE', 'NKQ*', 'MOT*', 'CTRL', 'FTS*', 'DSF',
                      'CST*', 'LOS*', 'UWB*']
    reduction_yes = set()
    reduction_no = set()
    reduction_count = {}
    ...
    if istlog and args.format == 'csv':  # we know our fields from the get-go
        try:
            ...
            for ttype in types:
                try:
                    typeClass = "MAVLink_{0}_message".format(ttype.lower())
                    if runningPython3:
                        fields += [ttype + '.' + x for x in
                                   inspect.getfullargspec(getattr(mavutil.mavlink, typeClass).__init__).args[1:]]
                    else:
                        fields += [ttype + '.' + x for x in
                                   inspect.getargspec(getattr(mavutil.mavlink, typeClass).__init__).args[1:]]
                    offsets[ttype] = currentOffset
        ...
        # CSV format outputs columnar data with a user-specified delimiter
            ...
            if timestamp == last_timestamp or last_timestamp is None:
                if isbin:
                    ...
                    newData = [
                        str(data[y.split('.')[-1]]) if y.split('.')[0] == m_type and y.split('.')[-1] in data else ""
                        for y in fields]
